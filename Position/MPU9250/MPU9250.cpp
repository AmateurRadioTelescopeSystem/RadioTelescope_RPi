...
void MPU9250::readAccelData(int16_t * destination)
{
	...
	destination[0] = ((int16_t)rx_buffer[0] << 8) | rx_buffer[1] ;
	destination[1] = ((int16_t)rx_buffer[2] << 8) | rx_buffer[3] ;
	destination[2] = ((int16_t)rx_buffer[4] << 8) | rx_buffer[5] ;
}
...
void MPU9250::readMagData(int16_t * destination)
{
	...
	if (readByte(AK8963_ADDRESS, AK8963_ST1) & 0x01)
	{
		...
		uint8_t c = rx_buffer[6];
		// Check if magnetic sensor overflow set, if not then report data
		if (!(c & 0x08))
		{
			// Turn the MSB and LSB into a signed 16-bit value
			destination[0] = ((int16_t)rx_buffer[1] << 8) | rx_buffer[0];
			// Data stored as little Endian
			destination[1] = ((int16_t)rx_buffer[3] << 8) | rx_buffer[2];
			destination[2] = ((int16_t)rx_buffer[5] << 8) | rx_buffer[4];
		}
	}
}
...
int16_t MPU9250::readTempData()
{
	...
	return ((int16_t)rx_buffer[0] << 8) | rx_buffer[1];
}
...
void MPU9250::initAK8963(double * destination)
{
	...
	destination[0] =  (double)(rx_buffer[0] - 128)/256. + 1.;
	destination[1] =  (double)(rx_buffer[1] - 128)/256. + 1.;
	destination[2] =  (double)(rx_buffer[2] - 128)/256. + 1.;
	writeByte(AK8963_ADDRESS, AK8963_CNTL, 0x00); // Power down magnetometer
	usleep(10000);
...
void MPU9250::calibrateMPU9250(double * gyroBias, double * accelBias)
{
...
// deviation is a pass.
void MPU9250::MPU9250SelfTest(double * destination)
{
	...
	factoryTrim[0] = (double)(2620/1<<FS)*(pow(1.01 ,((double)selfTest[0] - 1.0) ));
	// FT[Ya] factory trim calculation
	factoryTrim[1] = (double)(2620/1<<FS)*(pow(1.01 ,((double)selfTest[1] - 1.0) ));
	// FT[Za] factory trim calculation
	factoryTrim[2] = (double)(2620/1<<FS)*(pow(1.01 ,((double)selfTest[2] - 1.0) ));
	// FT[Xg] factory trim calculation
	factoryTrim[3] = (double)(2620/1<<FS)*(pow(1.01 ,((double)selfTest[3] - 1.0) ));
	// FT[Yg] factory trim calculation
	factoryTrim[4] = (double)(2620/1<<FS)*(pow(1.01 ,((double)selfTest[4] - 1.0) ));
	// FT[Zg] factory trim calculation
	factoryTrim[5] = (double)(2620/1<<FS)*(pow(1.01 ,((double)selfTest[5] - 1.0) ));
	...
	for (int i = 0; i < 3; i++)
	{
		...
		destination[i] = 100.0 * ((double)(aSTAvg[i] - aAvg[i])) / factoryTrim[i]
		  - 100.;
		// Report percent differences
		destination[i+3] = 100.0*((double)(gSTAvg[i] - gAvg[i])) / factoryTrim[i+3]
		  - 100.;
	}
}
...
void MPU9250::magCalMPU9250(double * bias_dest, double * scale_dest)
{
	...
	std::cout << "Mag Calibration: Wave device in a figure 8 until done!" << std::endl;
	std::cout << "  4 seconds to get ready followed by 15 seconds of sampling)" << std::endl;
	usleep(4000000);
	...
	bias_dest[0] = (double)mag_bias[0] * mRes * factoryMagCalibration[0];
	bias_dest[1] = (double)mag_bias[1] * mRes * factoryMagCalibration[1];
	bias_dest[2] = (double)mag_bias[2] * mRes * factoryMagCalibration[2];
...
int8_t MPU9250::i2cAddr(int devAddress)
{
...
uint8_t MPU9250::writeByte(uint8_t devAddress, uint8_t registerAddress, uint8_t data)
{
	i2cAddr(devAddress);
...
uint8_t MPU9250::readByte(uint8_t devAddress, uint8_t registerAddress)
{
	i2cAddr(devAddress);
...
uint8_t MPU9250::readBytes(uint8_t devAddress, uint8_t registerAddress, uint8_t count,
		uint8_t *dest)
{
	i2cAddr(devAddress);
	uint8_t buf[1] = {registerAddress};
	uint8_t bytes_read;
	...
	if(!dest)
		bytes_read = read(i2c_descriptor, rx_buffer, count);
	else
		bytes_read = read(i2c_descriptor, dest, count);
